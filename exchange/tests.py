...
from exchange.controllers import get_allocations_for_subject, get_available_exchanges, \
    ...
    number_of_students_in_allocation, get_subject_exchanges, get_student_subject_other_allocations, \
    is_exchange_acceptable
from exchange.models import Exchange, FormProcessingError, ExchangeType, SubjectPreference
...
class BaseTestCase(TestCase):
    ...
    #                             SS,         MO,         JM,         DG,         NA
    STUDENT_ENROLLMENT_NUMBERS = ["63120284", "63120294", "63130144", "63120183", "63120220"]
    ...
    def __init__(self, methodName='runTest'):
        ...
        if not self._has_keepdb():
            print("Tests must be run with --keepdb to preserve the test database between runs.")
        ...
        if not self._has_serialize_false():
            print("Tests must be run with DATABASES[dbname]['TEST']['SERIALIZE'] = False in settings.")
            print("Otherwise, the tests take several tens of orders of magnitude longer.")
            print("See: https://docs.djangoproject.com/en/2.0/ref/settings/#serialize")
            raise Exception("Running tests with DATABASES->TEST->SERIALIZE is mandatory.")
    ...
    def _has_serialize_false(cls):
        return any(settings.DATABASES) and all(db.get("TEST", {}).get("SERIALIZE", True) == False
    ...
    def setUpClass(cls):
        ...
        if not cls._has_data():
            ...
            for db_name in cls._databases_names(include_mirrors=False):
                ...
                try:
                    call_command('loaddata', cls.PERSISTENT_FIXTURE, **{
                        'verbosity': 0,
                        'commit': False,
                        'database': db_name,
                    })
    ...
    def setUpTestData(cls):
        ...
        cls.known_student_student = Student.objects.get(studentId=BaseTestCase.KNOWN_STUDENT[1])
        cls.students = [Student.objects.get(studentId=studentId) for studentId in
                        BaseTestCase.STUDENT_ENROLLMENT_NUMBERS]
        cls.subjects = [Subject.objects.get(code=code) for code in BaseTestCase.SUBJECT_CODES]
        cls.teachers = [Teacher.objects.get(code=code) for code in BaseTestCase.TEACHER_CODES]

        # additional processing based on the input data
        cls.subjects_lab_allocations = [get_allocations_for_subject(cls.timetable, s, ["LAB", "LV", "AV"])
                                        for s in cls.subjects]

        assert all(len(sals) >= 2 for sals in cls.subjects_lab_allocations), \
...
class ExchangeUtilsTestCase(BaseTestCase):
    ...
    def test_teacher_teaches_subject_yes(self):
        self.assertTrue(teacher_teaches_subject(self.timetable, self.teachers[0], self.subjects[0]))

    def test_teacher_teaches_subject_no(self):
        self.assertFalse(teacher_teaches_subject(self.timetable, self.teachers[1], self.subjects[0]))

    def test_get_allocations_for_subject(self):
        allocations_all = get_allocations_for_subject(self.timetable, self.subjects[0])
        allocations_labs = get_allocations_for_subject(self.timetable, self.subjects[0],
                                                       activity_types=["LAB", "LV", "AV"])
        allocations_lectures = get_allocations_for_subject(self.timetable, self.subjects[0],
                                                           activity_types=["P"])
        ...
        self.assertEqual(len(allocations_all), len(allocations_labs) + len(allocations_lectures))

    def test_get_allocations_for_subject_null_timetable(self):
        with self.assertRaises(ValueError):
            get_allocations_for_subject(None, self.subjects[0], activity_types=["LAB", "LV", "AV"])

    def test_get_allocations_for_subject_null_subject(self):
        with self.assertRaises(ValueError):
            get_allocations_for_subject(self.timetable, None, activity_types=["LAB", "LV", "AV"])
    ...
    def test_parse_student_from_ambiguous_identifier_email(self):
        self.assertEqual(self.students[0], parse_student_from_ambiguous_identifier("ss3055@student.uni-lj.si"))

    def test_parse_student_from_ambiguous_identifier_id(self):
        self.assertEqual(self.students[0], parse_student_from_ambiguous_identifier("63120284"))
    ...
    def test_get_current_student_subject_allocation_null_timetable(self):
        with self.assertRaises(ValueError):
            get_current_student_subject_allocation(None, self.students[0], self.subjects[0], ["LAB", "LV", "AV"])

    def test_get_current_student_subject_allocation_null_student(self):
        with self.assertRaises(ValueError):
            get_current_student_subject_allocation(self.timetable, None, self.subjects[0], ["LAB", "LV", "AV"])
    ...
    def test_get_student_subject_other_allocations_null_timetable(self):
        with self.assertRaises(ValueError):
            get_student_subject_other_allocations(None, self.students[0], self.subjects[0], ["LAB", "LV", "AV"])

    def test_get_student_subject_other_allocations_null_student(self):
        with self.assertRaises(ValueError):
            get_student_subject_other_allocations(self.timetable, None, self.subjects[0], ["LAB", "LV", "AV"])

    def test_get_student_subject_other_allocations_null_subject(self):
        with self.assertRaises(ValueError):
            get_student_subject_other_allocations(self.timetable, self.students[0], None, ["LAB", "LV", "AV"])
...
class ExchangeTestCase(BaseTestCase):
    ...
    @classmethod
    def _get_student_subject_allocation_inverse(cls, timetable, student, subject, activity_types, skip=0):
        ...
        """
        allocations = get_student_subject_other_allocations(timetable, student, subject, activity_types)
    ...
    def _perform_exchange_pair_success_test(self, exchange_left, exchange_right, exchange_left_type, exchange_right_type):
        ...
        self.assertEqual(globally_processed_exchanges_before + 2, globally_processed_exchanges_after)
    ...
    def test_get_student_exchanges(self):
        ...
        for e in exchanges:
            self.assertTrue(e.initiator_student == student or e.requested_finalizer_student == student)
    ...
    def test_request_offer_regular(self):
        self._perform_exchange_pair_success_test(self.exchanges[0], self.exchanges[1],
                                                 ExchangeType.REQUEST_OFFER, ExchangeType.REQUEST_OFFER)

    def test_request_offer_regular_inverse(self):
        self._perform_exchange_pair_success_test(self.exchanges[1], self.exchanges[0],
                                                 ExchangeType.REQUEST_OFFER, ExchangeType.REQUEST_OFFER)

    def test_request_offer_regular_endtoend(self):
        self._perform_exchange_pair_success_test_endtoend(self.exchanges[0], self.exchanges[1],
                                                          ExchangeType.REQUEST_OFFER, ExchangeType.REQUEST_OFFER)

    def test_request_offer_regular_endtoend_inverse(self):
        self._perform_exchange_pair_success_test_endtoend(self.exchanges[1], self.exchanges[0],
                                                          ExchangeType.REQUEST_OFFER, ExchangeType.REQUEST_OFFER)
    ...
    def test_request_offer_no_matches_endtoend(self):
        self._perform_no_matches_endtoend_test(self.exchanges[2], ExchangeType.REQUEST_OFFER)

    def test_request_offer_with_teacher_offer(self):
        self._perform_exchange_pair_success_test(self.exchanges[3], self.exchanges[4],
                                                 ExchangeType.REQUEST_OFFER, ExchangeType.TEACHER_OFFER)

    def test_request_offer_with_teacher_offer_inverse(self):
        self._perform_exchange_pair_success_test(self.exchanges[4], self.exchanges[3],
                                                 ExchangeType.TEACHER_OFFER, ExchangeType.REQUEST_OFFER)

    def test_request_offer_with_teacher_offer_endtoend(self):
        self._perform_exchange_pair_success_test_endtoend(self.exchanges[3], self.exchanges[4],
                                                          ExchangeType.REQUEST_OFFER, ExchangeType.TEACHER_OFFER)

    def test_request_offer_with_teacher_offer_endtoend_inverse(self):
        self._perform_exchange_pair_success_test_endtoend(self.exchanges[4], self.exchanges[3],
                                                          ExchangeType.TEACHER_OFFER, ExchangeType.REQUEST_OFFER)

    def test_specified_student(self):
        self._perform_exchange_pair_success_test(self.exchanges[6], self.exchanges[7],
                                                 ExchangeType.SPECIFIC_STUDENT, ExchangeType.SPECIFIC_STUDENT)

    def test_specified_student_inverse(self):
        self._perform_exchange_pair_success_test(self.exchanges[7], self.exchanges[6],
                                                 ExchangeType.SPECIFIC_STUDENT, ExchangeType.SPECIFIC_STUDENT)

    def test_specified_student_endtoend(self):
        self._perform_exchange_pair_success_test_endtoend(self.exchanges[6], self.exchanges[7],
                                                          ExchangeType.SPECIFIC_STUDENT, ExchangeType.SPECIFIC_STUDENT)

    def test_specified_student_endtoend_inverse(self):
        self._perform_exchange_pair_success_test_endtoend(self.exchanges[7], self.exchanges[6],
                                                          ExchangeType.SPECIFIC_STUDENT, ExchangeType.SPECIFIC_STUDENT)

    def test_free_change_no_create(self):
        exchanges_before = Exchange.objects.count()
        self._perform_exchange_pair_success_test(self.exchanges[9], self.exchanges[10],
                                                 ExchangeType.FREE_CHANGE, ExchangeType.REQUEST_OFFER)
        exchanges_after = Exchange.objects.count()
        self.assertEqual(exchanges_before, exchanges_after)

    def test_free_change_no_create_inverse(self):
        exchanges_before = Exchange.objects.count()
        self._perform_exchange_pair_success_test(self.exchanges[10], self.exchanges[9],
                                                 ExchangeType.REQUEST_OFFER, ExchangeType.FREE_CHANGE)
        exchanges_after = Exchange.objects.count()
        self.assertEqual(exchanges_before, exchanges_after)

    def test_free_change_no_create_endtoend(self):
        exchanges_before = Exchange.objects.count()
        self._perform_exchange_pair_success_test_endtoend(self.exchanges[9], self.exchanges[10],
                                                          ExchangeType.FREE_CHANGE, ExchangeType.REQUEST_OFFER)
        exchanges_after = Exchange.objects.count()
        self.assertEqual(exchanges_before + 1, exchanges_after)
    ...
    def test_free_change_no_create_endtoend_inverse(self):
        exchanges_before = Exchange.objects.count()
        self._perform_exchange_pair_success_test_endtoend(self.exchanges[10], self.exchanges[9],
                                                          ExchangeType.REQUEST_OFFER, ExchangeType.FREE_CHANGE)
        exchanges_after = Exchange.objects.count()
        self.assertEqual(exchanges_before + 1, exchanges_after)
    ...
    def test_free_change_yes_create_chain_process(self):
        exchanges_before = Exchange.objects.count()
        self._perform_exchange_pair_success_test(self.exchanges[11], self.exchanges[12],
                                                 ExchangeType.FREE_CHANGE, ExchangeType.REQUEST_OFFER)
        exchanges_after = Exchange.objects.count()
    ...
    def test_free_change_yes_create_chain_process_inverse(self):
        exchanges_before = Exchange.objects.count()
        self._perform_exchange_pair_success_test(self.exchanges[12], self.exchanges[11],
                                                 ExchangeType.REQUEST_OFFER, ExchangeType.FREE_CHANGE)
        exchanges_after = Exchange.objects.count()
    ...
    def test_free_change_yes_create_chain_process_endtoend(self):
        exchanges_before = Exchange.objects.count()
        self._perform_exchange_pair_success_test_endtoend(self.exchanges[11], self.exchanges[12],
                                                          ExchangeType.FREE_CHANGE, ExchangeType.REQUEST_OFFER,
    ...
    def test_free_change_yes_create_chain_process_endtoend_inverse(self):
        exchanges_before = Exchange.objects.count()
        self._perform_exchange_pair_success_test_endtoend(self.exchanges[12], self.exchanges[11],
                                                          ExchangeType.REQUEST_OFFER, ExchangeType.FREE_CHANGE,
                                                          additional_processed_exchanges=2)
        exchanges_after = Exchange.objects.count()
        self.assertEqual(exchanges_before + 2, exchanges_after)
    ...
    def test_disallow_exchange_for_subject_hard(self):
        preference, preference_created = SubjectPreference.objects.get_or_create(subject=self.subjects[0])
    ...
    def test_teacher_offer_for_other_student_no_matches_endtoend(self):
        self._perform_no_matches_endtoend_test(self.exchanges[13], ExchangeType.TEACHER_OFFER)

    def test_teacher_offer_for_other_student_no_matches_endtoend_inverse(self):
        self._perform_no_matches_endtoend_test(self.exchanges[14], ExchangeType.REQUEST_OFFER)
