...
class AllocationChoiceField(forms.ModelChoiceField):
    def __init__(self, allocation_queryset, *args, **kwargs):
        if allocation_queryset:
            allocation_queryset = allocation_queryset.order_by("-day", "start", "classroom")
        super().__init__(allocation_queryset, *args, empty_label="Select other instance", **kwargs)
...
class ExchangeCreationForm(forms.Form):
    ...
    requested_student = forms.CharField(required=False, label="Request a specific student to exchange with you",
                                        widget=forms.TextInput(attrs={"placeholder": "Email or enrollment number"}))

    def __init__(self, subject_available_allocation_map, subject_attending_allocation_map, *args, **kwargs):
        ...
        for subject in subject_available_allocation_map:
            ...
            self.fields[field_key] = AllocationChoiceField(available_allocations, required=False,
                                                           label="{}: swap your {} at {} in {} for"
                                                           .format(subject.name.strip(), attending_allocation.day,
...
class SubjectPreferenceForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "exchange_deadline": forms.TextInput(attrs={"placeholder": "e.g. 2018-02-14"})
        }
...
class StudentSelectionField(forms.ModelChoiceField):
    ...
    def label_from_instance(self, student):
        return "{} {} ({})".format(self._fix_name_caps(student.name), self._fix_name_caps(student.surname), student.studentId)
...
class TeacherExchangeCreationForm(forms.Form):
    ...
    def __init__(self, available_allocations, *args, **kwargs):
        ...
        initial_allocation_from = (kwargs or {}).get('initial', {}).get('current_student_allocation', None)

        self.fields["timetable"].queryset = Timetable.objects.filter(id=initial_timetable.id if initial_timetable else -1)
        self.fields["teacher"].queryset = Teacher.objects.filter(id=initial_teacher.id if initial_teacher else -1)
        self.fields["student"].queryset = Student.objects.filter(id=initial_student.id if initial_student else -1)
        self.fields["current_student_allocation"].queryset = Allocation.objects.filter(id=initial_allocation_from.id
                                                                                       if initial_allocation_from else -1)
        self.fields["requested_student_allocation"] = AllocationChoiceField(available_allocations)
    ...
    def get_subject_transfer_map(self):
        ...
        subject = Activity.from_timetable_activity(allocation.activityRealization.activity).subject
    ...
    def clean(self):
        ...
        requested_student_allocation = self.cleaned_data.get("requested_student_allocation")
        if not (timetable and teacher and student and current_student_allocation and requested_student_allocation):
            return super_result

        # ensure allocations are for the same subject
        subject_from = Activity.from_timetable_activity(current_student_allocation.activityRealization.activity).subject
        subject_to = Activity.from_timetable_activity(requested_student_allocation.activityRealization.activity).subject
        if subject_from != subject_to:
            raise forms.ValidationError("The allocations do not belong to the same subject.")

        # ensure the student attends the source allocation
        student_allocation = get_current_student_subject_allocation(timetable, student, subject_from, ["LAB", "LV", "AV"])
        if current_student_allocation != student_allocation:
            raise forms.ValidationError("The student does not currently attend the source allocation.")
