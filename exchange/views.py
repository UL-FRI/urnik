...
from exchange.forms import ExchangeCreationForm, SubjectPreferenceForm, StudentSelectionForm, \
...
def landing_student(request, timetable_slug):
    ...
    for s in student_subjects:
        ...
        try:
            a = get_current_student_subject_allocation(selected_timetable, student, s, ["LAB", "LV", "AV"])
        except:
            continue
        available_subject_exchanges_allocations.append((s, get_subject_exchanges(selected_timetable, s), a))

    return render(request, "exchange/student_main.html", {
...
def landing_teacher(request, timetable_slug):
    ...
    return render(request, "exchange/teacher_main.html", {
...
def subject_management(request, timetable_slug, subject_code):
    ...
    subject_students = subject.get_enrolled_students(selected_timetable).order_by('surname')
    ...
    if request.method == "POST":
        subject_preference_form = SubjectPreferenceForm(data=request.POST, instance=subject_preference)
    ...
    return render(request, "exchange/teacher_subject_management.html", {
        "selected_timetable": selected_timetable,
        "subject": subject,
...
def create_exchange_student(request, timetable_slug):
    ...
    for subject in subjects:
        ...
        try:
            student_allocation = get_current_student_subject_allocation(selected_timetable, student, subject, activity_types)
            allocations = get_student_subject_other_allocations(selected_timetable, student, subject, activity_types)
    ...
    if request.method == "POST":
        form = ExchangeCreationForm(subject_available_allocation_map, subject_attending_allocation_map, data=request.POST)
        if form.is_valid():
            try:
                requested_student_string = form.get_requested_student()
                requested_student = None
                if requested_student_string:
                    requested_student = parse_student_from_ambiguous_identifier(requested_student_string)
                any_fulfilled = process_new_exchange_request(selected_timetable, student, requested_student,
                                                             form.get_subject_transfers(keep_empty=False))
            except FormProcessingError as e:
                return render(request, "exchange/exchange_create_result.html", {
                    "selected_timetable": selected_timetable,
                    "header": e.header,
                    "message": e.message
                })
        ...
        form = ExchangeCreationForm(subject_available_allocation_map, subject_attending_allocation_map)
    ...
    if exchange.is_finalized():
        ...
        try:
            any_fulfilled = process_new_exchange_request(
                selected_timetable,
                student,
                exchange.initiator_student if exchange.get_type() == ExchangeType.SPECIFIC_STUDENT else None,
...
def create_exchange_teacher(request, timetable_slug, subject_code):
    ...
    try:
        student_allocation = get_current_student_subject_allocation(selected_timetable, selected_student,
                                                                    subject, activity_types)
        available_allocations = get_student_subject_other_allocations(selected_timetable, selected_student,
                                                                      subject, activity_types)
    except Allocation.DoesNotExist:
        raise Http404("The student does not attend the subject or no allocations have been found.")

    exchange_creation_form = TeacherExchangeCreationForm(available_allocations, data=request.POST or None, initial={
    ...
    if request.method == "GET":
        ...
        if exchange_creation_form.is_valid():
            any_fulfilled = process_new_exchange_request(
                ...
                force_allocation_from=exchange_creation_form.cleaned_data["requested_student_allocation"]
            )
            if any_fulfilled:
                header = "Exchange fulfilled immediately!"
