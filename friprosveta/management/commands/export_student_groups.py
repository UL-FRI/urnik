...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        assert len(args) >= 2, "Usage: timetable_slug subject_code [subject_code2] [subject_code3...]"
    ...
    def print_groups(self, tt, subjects):
        for subject in subjects:
            ...
            for allocation in tt.allocations.filter(
                    activityRealization__activity__activity__subject=subject).distinct():
                s = "; ".join(["{0} {1}".format(t.user.first_name, t.user.last_name) for t in
                                allocation.activityRealization.teachers.all()])
                print("    {0} {1}-{2}, {3}: {4}".format(allocation.day, allocation.start, allocation.end,
                                                          allocation.classroom, s))
                students = set()
                for group in allocation.activityRealization.groups.distinct():
                    for student in group.students.all():
                        students.add('{1} {2} ({0})'.format(student.studentId, student.name, student.surname))
                print('        ' + '\n        '.join(students))
