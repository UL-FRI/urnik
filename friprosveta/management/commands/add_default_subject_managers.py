...
class Command(BaseCommand):
    def addCatedraHeadsAndDeputies(self, obdobja, clear_managers):
        for s in friprosveta.models.Subject.objects.all():
            if clear_managers:
                s.managers.clear()
            # print s
            dpt = maroltnajave.models.Delavecpredmett.objects.filter(subject__sifra = s.code, type=1, valid=True, obdobje__in = obdobja)
        # ce ne znamo najti katedre, med managerje dodamo nosilca
            mgrs = set()
            if len(dpt) == 0:
                for t in friprosveta.models.Teacher.objects.filter(activities__activity__subject = s, activities__type='P').distinct():
...
#            print "    ", mgrs
    ...
    def handle(self, *args, **options):
        ...
        self.addCatedraHeadsAndDeputies(obdobja, True)
        self.addStaff()
