...
pset is also given, otherwise it is silently ignored.'),
    ...
    def handle(self, *args, **options):
        ...
        if options['activity_id'] is not None:
            ...
            self.tag_activities(activityset.activities.get(pk=options['activity_id'][0]), year)
        else:
        ...
        if options['preferenceset_slug'] is not None:
            ...
            preferenceset = PreferenceSet.objects.get(slug=options['preferenceset_slug'][0])
    ...
    def tag_activities(self, activities, year):
        ...
        for activity in activities:
            ...
            if activity.type == 'P':
                try:
                    if activity.activity.subject.is_obligatory(year, studij=studij, najave=najave):
