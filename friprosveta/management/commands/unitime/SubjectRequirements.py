...
def subjectRequirements(tt):
    ...
    for subject in tt.subjects:
        ...
        for activity in subject.activities.filter(activityset=tt.activityset):
            ...
            if subpart_id == None: continue

            # delete_query = u"DELETE FROM room_feature_pref WHERE owner_id={0}".format(subpart_id)
            # db.execute(delete_query)
            # Do NOT delete old preferences, just add missing ones
            query = u"SELECT room_feature_id FROM room_feature_pref WHERE owner_id={0}".format(subpart_id)
            db.execute(query)
            feature_ids = [fid[0] for fid in db.fetchallrows()]
            ...
            for requirement in activity.requirements.all():
                ...
                if db.rowcount == 0:
                    continue
                assert db.rowcount == 1, u"There should be exactly one preference for {0}".format(name)
                feature_id = db.fetchnextrow()[0]
                if feature_id not in feature_ids:
                    # print "Inserting feature", activity.name
                    query = u"""INSERT room_feature_pref (uniqueid,
