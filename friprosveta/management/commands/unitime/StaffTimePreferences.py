...
def staff_time_preferences(tt):
    ...
    for teacher in tt.teachers.all():
        preferences = (
            PreferenceLevel.Neutral * 12 * 24 * 7
        )  # 5 minute slot, 7 days, 24 hours
        for time_preference in TeacherTimePreference.objects.filter(
            preferenceset=tt.preferenceset, teacher=teacher
        ):
        ...
        if db.rowcount != 1:
            print(f"Error with {teacher}, skipping")
            continue

        print(f"Processing {teacher}")
        assert (
            db.rowcount == 1
        ), "More than one entry in unitime for teacher {0} with id {1}".format(
            teacher, teacher.id
        )
        teacher_id = db.fetch_next_row()[0]

        remove_previous_preference_query = (
            """DELETE FROM timetable.time_pref WHERE owner_id={0}""".format(teacher_id)
        )
        ...
        add_preference_query = """INSERT INTO timetable.time_pref 
                   ...
                   VALUES ({0}, 1, '{1}', null, {2})""".format(
            teacher_id, preferences, next_id
        )
