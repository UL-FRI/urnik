...
default_lecture_time_pattern = ((PreferenceLevel.Prohibited * 1) +
...
default_exercise_time_pattern = ((PreferenceLevel.StronglyDiscouraged * 1) +
...
def create_default_time_patterns(tt):
    patterns = [(1, 720), (1, 240), (1, 180), (1, 120), (1, 60),]
    db = Database()
...
def combineTimePreferences(preference1, preference2):
    ...
    """
    order = [PreferenceLevel.Prohibited,
    ...
    assert len(preference1) == len(preference2), "Preferences should be of the same length\n{0}\n{1}".format(preference1, preference2)
    merged = ''
...
#             db.execute(query)            
#             for class_id in db.fetchallrows():
...
#                 db.execute(delete_class_preference_query)
            ...
            preference_query = "SELECT preference FROM timetable.time_pref WHERE owner_id={0}".format(subpart_id)
            db.execute(preference_query)
            for preference in db.fetchallrows():
                preference = preference[0]
                if len(database_preference) == len(preference):
                    database_preference = combineTimePreferences(database_preference,
                                                                 preference)
            pattern = default_exercise_time_pattern
            if activity.type == 'P':
                pattern = default_lecture_time_pattern
            pattern = pattern[:14-activity.duration]*5 + PreferenceLevel.Prohibited*(14-activity.duration)*2
            pattern = combineTimePreferences(pattern, database_preference)
            delete_subpart_preferrence_query = ("DELETE FROM timetable.time_pref "
                                                "WHERE owner_id={0}").format(subpart_id)
