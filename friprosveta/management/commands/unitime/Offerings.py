...
def classes_for_subject(subject_code, itype):
    ...
    co = CourseOffering.objects.filter(subject_area__subject_area_abbreviation=subject_code)
    if co.count() != 1:
        return []
    co = co.get()
    return Class.objects.filter(subpart__config__instr_offr=co.instr_offr, subpart__itype__itype=itype).distinct()
...
def offerings(tt, campus, term, year):
    ...
    for subject in tt.subjects.all():
        ...
        for activity in subject.activities.filter(activityset=activityset).order_by('-duration'):
            ...
            # print id_pool
            if None in id_pool: id_pool[''] = id_pool[None]
            ...
            try:
                ...
                for key in subject_teachers.keys():
                    ...
                    class_size = ACTIVITY_TEACHERS_SIZE[(activity.type,
                                                         instruction_style[1])][1]
                    if class_size == 0:
                        class_size = enrolled_limit if enrolled_limit else projected_limit
