...
class Command(BaseCommand):
    ...
    def sync_subjects(self):
        ...
        for subject in subjects:
            ...
            fri_subjects_count = Subject.objects.filter(code=subject["sifra"]).count()
            logger.debug(
                "{0} matching subjects found in our database".format(fri_subjects_count)
            )
            assert (
                fri_subjects_count <= 1
            ), "More than one subject \
with code {0} in our database.".format(
                subject["sifra"]
            )
            if fri_subjects_count == 0:
                ...
                logger.info(
                    "Created subject {1} ({0})".format(subject["sifra"], subject_name)
                )
                if subject["sifra"] in subjects_added:
                    logger.error(f"Duplicated subject code {subject['sifra']}")
                subjects_added[subject["sifra"]] = Subject(
                    code=subject["sifra"], name=subject_name
                )
            else:
