...
class Command(BaseCommand):
    ...
    def split_activity(self, tt, activity, splits):
        if activity.duration != sum(splits):
            raise Exception(
                "Activity {0} with duration {1} cannot be split into {3}".format(
        ...
        with transaction.atomic():
            ...
            t12urmed = timetable.models.Tag(
                name="12 ur med {0}".format(act.subject.short_name)
            )
            t12urmed.save()
            t12urmed.activities.add(*all_activities)
            tvp = timetable.models.TagValuePreference(
                tag=t12urmed,
                value=13,
                weight=1.0,
                level="WANT",
