...
is read from studis. "
    ...
    def handle(self, *args, **options):
        ...
        headers = {"Content-Type": "application/json", "AUTHENTICATION_TOKEN": token}
        ...
        for izvajanje in izvajanja:
            study, classyear = get_study_classyear(
                studijsko_drevo, izvajanje["predmetnik"]
            )
            if study is None:
                continue
        ...
        if options["subject"] is not None:
            subjects = friprosveta.models.Subject.objects.filter(
                code=options["subject"]
            )

        for subject in subjects:
            try:
                studis_activity = activities[subject.code]
                self.update_groups_for_subject(
                    subject,
                    timetable,
                    options["testrun"],
                    studis_activity,
                ...
                print(
                    "Error updating groups for subject {0}: {1}".format(
                        subject, repr(e)
                    ).encode("utf-8")
                )
                traceback.print_exc()
    ...
    @transaction.atomic
    def update_groups_for_subject(
        self,
        subject,
        timetable,
        testrun,
        studis_activity,
        safe_to_delete_studies=["PAD", "UN-RI"],
        never_delete_studies=[
            "FKKT-UNI",
            "FKKT-VS",
            "BUN-UI",
            "BUN-MM",
        ...
        oldutils = Utils()
        assert (
            timetable.subjects.filter(id=subject.id).count() == 1
        ), "Subject {0} must exist \
in the timetable {1}".format(
            subject, timetable
        ).encode(
            "utf-8"
        )
        for activity in subject.activities.filter(activityset=timetable.activityset):
            ...
            groups = oldutils.updateGroupsForActivity(
                activity=activity, timetable=timetable, studis_studies=studis_activity
            )
            updated_groups_ids = []
            if activity.type == "P":
            ...
            for group in groups_to_remove.all():
                if group.study in never_delete_studies:
                    print(
                        "Group {0} study in never_delete_studies, not removing".format(
                            group
                        )
                    )
                    continue
                if (
                    friprosveta.models.StudentEnrollment.objects.filter(
                        ...
                        study__short_name=group.study,
                    ).count()
                    == 0
                ):
                    if group.study in safe_to_delete_studies or raw_input(
                        "No enrolment data found for group {0}. Delete (Y/n)? ".format(
                            group
                        )
                    ) in ["y", "Y"]:
                        print("Deleting")
