    ...
    def handle(self, *args, **options):
        ...
        for predmet_cikli in predmeti_cikli:
            if predmet_cikli["predmet_sifra"] not in subject_codes:
                logger.debug(
                    "Subject {} not in subject_codes".format(
                        predmet_cikli["predmet_sifra"]
                    )
                )
                continue
            if str(predmet_cikli["izvajanje_id"]) not in izvajanja_ids:
                ...
                logger.debug(
                    "Subject {} not in this semester".format(
                        predmet_cikli["predmet_sifra"]
                    )
                )
                continue
            TeacherSubjectCycles.objects.filter(
                timetable_set=timetable_set, subject_code=predmet_cikli["predmet_sifra"]
            ).delete()
