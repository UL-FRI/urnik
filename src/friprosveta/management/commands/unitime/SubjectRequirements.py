...
def subject_requirements(tt):
    ...
    for subject in tt.subjects:
        ...
        print("Activityset", tt.activityset)
        ...
        for activity in subject.activities.filter(activityset=tt.activityset):
            ...
            if db.rowcount != 1:
                print(
                    "There should be exactly one sheduling subpart for activity {0}".format(
                        activity
                    )
                )
                continue
            ...
            query = "SELECT room_feature_id FROM room_feature_pref WHERE owner_id={0}".format(
                subpart_id
            )
            db.execute(query)
            feature_ids = [fid[0] for fid in db.fetch_all_rows()]

            print(activity.requirements.count())
            print(activity.requirements_per_student.count())

            for requirement in activity.requirements.all():
                name = requirement.name[:20]
                print("Processing requirement", name)
                feature_id_query = (
                    "SELECT uniqueid FROM room_feature WHERE label='{0}'".format(name)
                )
                ...
                assert (
                    db.rowcount == 1
                ), "There should be exactly one preference for {0}".format(name)
