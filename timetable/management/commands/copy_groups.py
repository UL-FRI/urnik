...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        assert len(args) >= 2, u"At least one destination and one source timetable must be specified."        
            ...
            for previous in source_timetables.all():
                if previous.subjects.filter(code=subject.code).count()==1:
                    ...
                    if pactivities.count() > 1: 
                        #print u"More than one activity for {0} of type {1} found in {2}".format(subject, activity.type, previous)
    ...
    def copy_group(self, old_group, new_groupset):
        assert new_groupset.groups.filter(name=old_group.name).count()<=1, u"More than one group with name {0} found".format(old_group.name)
        if new_groupset.groups.filter(name=old_group.name).count()==0:
            # Create new group
            new_group = Group(name=old_group.name, short_name=old_group.short_name, size=old_group.size, groupset=new_groupset)
            # Get parent
